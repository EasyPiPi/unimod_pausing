...
rate_tbls <- tibble(read_density = read_density,
       tbl = list(rate_tbls)) %>%
  unnest(cols = tbl)
...
rate_tbls <- rate_tbls %>%
  mutate(a = factor(a, levels = rate_ranges),
...
get_omega_lrt_results <- function(ctrl_tbl, test_tbl) {
  ...
  test_res <-
    ...
    mutate(sig = (p.adjust(p, method = "fdr") < .05),
           logp = log(p))
...
rc_tbl_gb <- rc_tbl_gb %>%
  mutate(lrt_a_fix_a = map(data, ~ do_lrt(.x, a = 1, b = 1, fix = "a", lrt = "a")),
...
p <- rc_tbl_gb %>% 
  ...
  labs(y = "Statistical Power", x = expression("True "*alpha*zeta),
       color = "Exp. Level", linetype = "Category",
       title = expression("LRT on "*omega)) +
  theme(plot.title = element_text(hjust = 0.5))
  
ggsave(file.path(figure_dir, paste0("lrt_omega_fix_beta", suffix, ".png")), plot = p,
       width = 8, height = 3)
...
p <- rc_tbl_gb %>% 
  ...
  # geom_line(aes(color = read_density, linetype = r)) +
  geom_line(aes(color = read_density)) +
  labs(y = "Statistical Power", x = expression("True "*beta*zeta),
       color = "Exp. Level", linetype = "Category",
       title = expression("LRT on "*omega)) +
  theme(plot.title = element_text(hjust = 0.5))

ggsave(file.path(figure_dir, paste0("lrt_omega_fix_alpha", suffix, ".png")), plot = p,
       width = 8, height = 3)
...
ggsave(file.path(figure_dir, paste0("lrt_beta_fix_alpha", suffix, ".png")), plot = p,
       width = 8, height = 3)
...
ggsave(file.path(figure_dir, paste0("lrt_beta_fix_alpha", suffix, "_grant.png")), plot = p,
       width = 4, height = 2.5)
...
p <- rc_tbl_gb %>%
  ...
  # geom_line(aes(color = read_density, linetype = r)) +
  geom_line(aes(color = read_density)) +
  labs(y = "False Positive Rate", x = expression("True "*alpha*zeta),
       color = "Exp. Level", linetype = "Category",
       title = expression("LRT on "*beta)) +
  theme(plot.title = element_text(hjust = 0.5))

ggsave(file.path(figure_dir, paste0("lrt_beta_fix_beta", suffix, ".png")), plot = p,
       width = 8, height = 3)

# Chi distribution for sanity check
y_int <-
  tibble("read_density" = str_remove(read_density, "lrt_"),
...
p <- rc_tbl %>%
  ...
  geom_hline(aes(yintercept = y, color = read_density), data = y_int,
             linetype="dashed") +
  labs(x = expression("True "*alpha*zeta), y = expression(chi), color = "Exp. Level")

ggsave(file.path(figure_dir, paste0("chi_distribution_with_targeted_read_density", suffix, ".png")), plot = p,
       width = 8, height = 3)
...
ggsave(file.path(figure_dir, paste0("lrt_omega_fix_beta_t_stats", suffix, ".png")), plot = p,
       width = 8, height = 3)
...
ggsave(file.path(figure_dir, paste0("lrt_omega_fix_alpha_t_stats", suffix, ".png")), plot = p,
       width = 8, height = 3)
...
ggsave(file.path(figure_dir, paste0("lrt_beta_fix_alpha_t_stats", suffix, ".png")), plot = p,
       width = 8, height = 3)
...
ggsave(file.path(figure_dir, paste0("lrt_beta_fix_beta_t_stats", suffix, ".png")), plot = p,
       width = 8, height = 3)
