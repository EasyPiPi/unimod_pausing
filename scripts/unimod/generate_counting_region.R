...
tss_length <- as.numeric(snakemake@params[["tss_length"]])
tts_length <- as.numeric(snakemake@params[["tts_length"]])
gb_min_length <- as.numeric(snakemake@params[["gb_min_length"]])
gb_max_length <- as.numeric(snakemake@params[["gb_max_length"]])
dist_to_tss <- as.numeric(snakemake@params[["dist_to_tss"]])
...
# tsn_in <- file.path(root_dir, "outputs/read_dt/max_tsn_per_gene_DLD1.rds")
...
# print(c(tss_length, tts_length, gb_min_length, gb_max_length, dist_to_tss))
...
save(bw_pause_filtered, bw_gb_filtered,
  bw_tts_filtered, count_grng,
  file = grng_out
)
