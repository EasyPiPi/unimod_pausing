...
if (length(idx1) > 0) {
  ...
  ggsave(file.path(result_dir, "fk_mean_differences.png"), plot = p,
         width = 6, height = 3)
  ...
  ggsave(file.path(result_dir, "fk_var_differences.png"), plot = p,
         width = 6, height = 3)
}
...
volcano_plot <- function(df, sig_p) {
  ...
  df %>% na.omit() %>%
    ...
    labs(x = expression(log[2]*"FC(Treated/Control)"),
         y = expression(-log[10]*"(padj)")) +
    cowplot::theme_cowplot()
}

p <- volcano_plot(omega_tbl, sig_p = sig_p)
ggsave(file.path(result_dir, "chi_lrt_volcano.png"), plot = p,
       width = 8, height = 6)

p <- volcano_plot(beta_tbl, sig_p = sig_p)
ggsave(file.path(result_dir, "beta_lrt_volcano.png"), plot = p,
       width = 8, height = 6)
...
# 
p <- lfc_tbl %>%
  ggscatter(x = "lfc_chi", y = "lfc_beta",
...
ggsave(file.path(result_dir, "lfc_chi_vs_beta.png"), plot = p,
       width = 6, height = 5)
# 
# p <- lfc_tbl %>%
...
ggsave(file.path(result_dir, "mean_vs_std_of_k.png"), plot = p,
       width = 9, height = 4)
