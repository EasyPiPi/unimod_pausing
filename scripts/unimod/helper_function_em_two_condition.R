...
get_maximization_h0 <- function(chi_hat, Xk1, Xk2, Yk1, Yk2, fk1, fk2, kmin, kmax) {

  fk1_ls <- get_fk(Xk1, Yk1, fk1, kmin, kmax)
  fk2_ls <- get_fk(Xk2, Yk2, fk2, kmin, kmax)

  beta <- chi_hat / (fk1_ls$t + fk2_ls$t)
...
main_EM_h0 <- function(fk_int, Xk1, Xk2, kmin, kmax, beta_int,
  ...
  for (i in 1:max_itr) {
    ...
    likelihoods[[i]] <-
      ...
      get_likelihood(beta = hats$beta, chi = chi_hat2, Xk = Xk2, Yk = Yk2, fk = hats$fk2)
